...
# Site
groups_site <- metadata_mid %>%
  select(Site) %>% 
  pull()

## Calculate multivariate dispersions
site_betadisper_mid <- betadisper(mpa3_species_decontam_unfilt_mid.euc, groups_site, bias.adjust =TRUE)
site_betadisper_mid

## Perform test
anova(site_betadisper_mid)

## Permutation test for F
permutest(site_betadisper_mid, pairwise = TRUE, permutations = 99)

## Tukey's Honest Significant Differences
site_betadisper_mid.HSD <- TukeyHSD(site_betadisper_mid)
plot(site_betadisper_mid.HSD)

## Plot the groups and distances to centroids on the
## first two PCoA axes
## with data ellipses instead of hulls
plot(site_betadisper_mid, ellipse = TRUE, hull = FALSE) # 1 sd data ellipse

## can also specify which axes to plot, ordering respected
# plot(site_betadisper_mid, axes = c(3,1), seg.col = "forestgreen", seg.lty = "dashed")

## Draw a boxplot of the distances to centroid for each group
boxplot(site_betadisper_mid)


# Region
groups_region <- metadata_mid %>%
  select(Region) %>% 
  pull()

## Calculate multivariate dispersions
region_betadisper_mid <- betadisper(mpa3_species_decontam_unfilt_mid.euc, groups_region, bias.adjust =TRUE)
region_betadisper_mid

## Perform test
anova(region_betadisper_mid)

## Permutation test for F
permutest(region_betadisper_mid, pairwise = TRUE, permutations = 99)

## Tukey's Honest Significant Differences
region_betadisper_mid.HSD <- TukeyHSD(region_betadisper_mid)
plot(region_betadisper_mid.HSD)

## Plot with data ellipses instead of hulls
plot(region_betadisper_mid, ellipse = TRUE, hull = FALSE) # 1 sd data ellipse

## Draw a boxplot of the distances to centroid for each group
boxplot(region_betadisper_mid)


# Time period
groups_tp <- metadata_mid %>%
  select(Time_period) %>% 
  pull()

## Calculate multivariate dispersions
tp_betadisper_mid <- betadisper(mpa3_species_decontam_unfilt_mid.euc, groups_tp, bias.adjust =TRUE)
tp_betadisper_mid

## Perform test
anova(tp_betadisper_mid)

## Permutation test for F
permutest(tp_betadisper_mid, pairwise = TRUE, permutations = 99)

## Tukey's Honest Significant Differences
tp_betadisper_mid.HSD <- TukeyHSD(tp_betadisper_mid)
plot(tp_betadisper_mid.HSD)

## Plot with data ellipses instead of hulls
plot(tp_betadisper_mid, ellipse = TRUE, hull = FALSE) # 1 sd data ellipse

## Draw a boxplot of the distances to centroid for each group
boxplot(tp_betadisper_mid)
