...
```{r}
# read in all contaminant tables
...
mpa3_sp_full.matrix <- as.matrix(mpa3_sp %>%
  select(-matches("ARS|EXB|LIB|CSN|CSL|CSD|CSS")) %>%
  gather("Library_ID","Counts",2:ncol(.)) %>%
  spread(Species,Counts) %>% 
  column_to_rownames("Library_ID"))
...
```{r sub10M_alpha}
# get the number of observed species
presabs_table_f <- mpa3_sub10M_sp %>%
  select(-matches("ARS|EXB|LIB|CSN|CSL|CSD|CSS")) %>%
  gather("Library_ID","Counts",2:ncol(.)) %>%
  spread(Species,Counts) %>%
  gather("Species","Counts",2:ncol(.)) %>%
...
# get the Shannon index
mpa3_sp_10M.matrix <- as.matrix(mpa3_sub10M_sp %>%
  select(-matches("ARS|EXB|LIB|CSN|CSL|CSD|CSS")) %>%
  gather("Library_ID","Counts",2:ncol(.)) %>%
  spread(Species,Counts) %>% 
  column_to_rownames("Library_ID"))
...
# ggsave("./06-publication/supplemental_figures/SXX11/adiv_sub_figure.pdf", plot = adiv_sub_figure, device = "pdf",
