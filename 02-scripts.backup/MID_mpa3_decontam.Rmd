...
We wont' add the ARS sample metadata from the DeepEvo project b/c they have only 2 species assigned to any of the 10 samples
```{r deep_evo_meta, eval = F}
...
ars_samples <- deep_evo_metadata %>%
  select(Library_ID) %>%
  mutate(Sample_or_Control = "Control",
         Site = "ARS")

# add ARS samples to the rest of the metadata table
full_metadata <- full_metadata %>%
  full_join(., ars_samples)
...
species_nt.pca <- mixOmics::pca(species_raw, ncomp = 3, logratio = 'CLR')
plot(species_nt.pca)
...
plot_pca(species_nt.pca, "PC1", "PC2", "Sample_or_Control")
plot_pca_names(species_nt.pca,  "PC1", "PC2", "Sample_or_Control")
...
species_mpa3_mid.dataframe <- mpa3_sp %>%
...
metadata_mid.decontam <- full_metadata %>%
  ...
  # these samples have no metadata
  filter(!str_detect(Library_ID, "LIB058.A0101|MID025.A0101|MID033.A0101|MID052.A0101|MID056.A0101|MID065.A0101|MID068.A0101|MID076.A0101|MID078.A0101")) %>%
  arrange(Library_ID) %>%
  distinct() %>%
  arrange(Site)
...
fwrite(contaminant_species_mpa3, file = "./05-results.backup/contaminant_species_mid_mpa3.tsv", sep="\t", quote=F)
...
# remove NA value samples from the input data frame
species_mpa3_rad.dataframe <- mpa3_sp %>%
  filter(!str_detect(Species, "Not assigned|Homo sapiens")) %>%
  select(-one_of(outliers_mpa3)) %>%
  select(-one_of(poor_samples_mpa3)) %>%
...
fwrite(contaminant_species_rad_mpa3, file = "./05-results.backup/contaminant_species_radcliffe_mpa3.tsv", sep="\t", quote=F)
...
# remove NA value samples from the input data frame
species_mpa3_kil.dataframe <- mpa3_sp %>%
  filter(!str_detect(Species, "Not assigned|Homo sapiens")) %>%
  select(-one_of(outliers_mpa3)) %>%
  select(-one_of(poor_samples_mpa3)) %>%
...
fwrite(contaminant_species_kil_mpa3, file = "./05-results.backup/contaminant_species_kilteasheen_mpa3.tsv", sep="\t", quote=F)
...
```{r iberia_data_prep}
# remove NA value samples from the input data frame
species_mpa3_ibe.dataframe <- mpa3_sp %>%
  filter(!str_detect(Species, "Not assigned|Homo sapiens")) %>%
  select(-one_of(outliers_mpa3)) %>%
  select(-one_of(poor_samples_mpa3)) %>%
...
fwrite(contaminant_species_ibe_mpa3, file = "./05-results.backup/contaminant_species_iberia_mpa3.tsv", sep="\t", quote=F)
