...
presabs_table <- all_species_decontam %>%
...
plot_box_whisker(all_species_decontam.os, "Observed_species", "Site", "Site")
...
os_plot <- ggplot(all_species_decontam.os, aes(x = Site, y = Observed_species, fill = Site)) +
  geom_boxplot(color = "grey30") +
...
presabs_table_f <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(-matches("EXB|LIB|CSN|CSL")) %>%
...
plot_box_whisker(all_species_decontam_filtered.os, "Observed_species", "Site", "Site")
...
filtered_os <- ggplot(all_species_decontam_filtered.os, aes(x = Site, y = Observed_species, fill = Site)) +
  geom_boxplot(color = "grey30") +
  geom_point(alpha = 0.5, color = "grey30", position = position_jitterdodge()) +
  scale_fill_manual(values = Site_colors) +
  theme_minimal(base_size = 18) +
...
all_species_decontam.matrix <- as.matrix(all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(-matches("EXB|LIB|CSN|CSL")) %>%
  gather("Library_ID","Counts",2:ncol(.)) %>%
...
plot_box_whisker(all_species_decontam.shannon, "Shannon_index", "Site", "Site")
...
all_species_decontam_filtered.matrix <- as.matrix(all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(-matches("EXB|LIB|CSN|CSL")) %>%
  adorn_totals(where = "col", name = "Sum_species.decontam") %>%
...
plot_box_whisker(all_species_decontam_filtered.shannon, "Shannon_index", "Site", "Site")
...
plot_box_whisker(evenness.species, "Pileou_evenness", "Site", "Site")
...
plot_box_whisker(evenness.species_filtered, "Pileou_evenness", "Site", "Site")
...
os_site <- plot_box_whisker(all_species_decontam_filtered.alpha, "Observed_species", "Site", "Site")
os_site
shannon_site <- plot_box_whisker(all_species_decontam_filtered.alpha, "Shannon_index", "Site", "Site")
shannon_site
pe_site <- plot_box_whisker(all_species_decontam_filtered.alpha, "Pileou_evenness", "Site", "Site")
pe_site
...
all_species_decontam_top50_all <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
...
all_species_decontam_top50_all_clr_smp_order <- all_species_decontam_top50_all_clr %>%
  ...
  gather("Species","CLR",2:ncol(.)) %>%
  spread(Species,CLR) %>%
  mutate(SmpOrder = as.character(seq(1,156))) %>%  
...
all_species_decontam_top50_all_clr_long <- all_species_decontam_top50_all_clr %>%
  ...
  inner_join(., Time_period_colors %>%
               as_data_frame(.) %>%
               rename(Color = value) %>%
               mutate(Time_period = c("Industrial", "Medieval","Extraction_blank","Library_blank")), by = "Time_period") %>%
...
all_species_decontam_top50_mid <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(matches("Species|MID")) %>%
  adorn_totals(where = "col", name = "Sum_species.decontam") %>%
...
all_species_decontam_top50_midV <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(matches("Species|MID")) %>%
  adorn_totals(where = "col", name = "Sum_species.decontam") %>%
...
all_species_decontam_top50_radV <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
  select(matches("Species|CS")) %>%
  select(-matches("CSS|CSD|CSN|CSL")) %>%
...
all_species_decontam_top50_kilV <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
...
all_species_decontam_top50_iberV <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
...
all_species_decontam_top50_each <- all_species_decontam %>%
  select(-one_of(outliers)) %>%
  select(-one_of(poor_samples)) %>%
...
all_species_decontam_top50_each_clr_order <- all_species_decontam_top50_each_clr %>%
  ...
  gather("Species","CLR",2:ncol(.)) %>%
  spread(Library_ID,CLR) %>%
  mutate(SpOrder = as.character(seq(1,64))) %>%
  select(Species, SpOrder, everything()) %>%
...
all_species_decontam_top50_each_clr_long <- all_species_decontam_top50_each_clr %>%
  ...
  inner_join(., Site_colors %>%
               as_data_frame(.) %>%
               rename(Color = value) %>%
               mutate(Site = c("Middenbeemster", "CMB", "Radcliffe", "Kilteasheen",
                                   "ELR", "IVE","Extraction_blank","Library_blank")), by = "Site") %>%
