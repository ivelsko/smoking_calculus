...
jae_table <- fread("~/projects1/microbiome_calculus/iberian/05-results.backup/Iberian_samples-controls_comparison_species.txt") %>%
  select(matches("Data|JAE")) %>%
  adorn_totals(where = "col") %>%
  filter(Total > 0) %>%
  select(-Total)

fwrite(jae_table, "./05-results.backup/jae_malt_species_raw.tsv")
...
jae_raw <- fread("./05-results.backup/jae_malt_species_raw.tsv")
...
all_species_decontam <- mid_species_decontam %>%
  ...
  full_join(., jae_species_decontam)
