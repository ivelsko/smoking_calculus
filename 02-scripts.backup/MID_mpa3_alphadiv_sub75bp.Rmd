...
mpa3_sub75bp_species_decontam.os %>%
  full_join(., mpa3_sub75bp_species_decontam_filtered.os %>%
...
```{r}
# compare difference in the number of species between filtered and unfiltered tables
filtered_sp_one <- mpa3_sub75bp_species_decontam.os %>%
  full_join(., mpa3_sub75bp_species_decontam_filtered.os %>%
              rename(os_filt = Observed_species), by = "Library_ID") %>%
  mutate(difference = Observed_species - os_filt) %>%
...
# compare difference in the number of species between filtered and unfiltered tables
filtered_sp <- mpa3_sub75bp_species_decontam.os %>%
  full_join(., mpa3_sub75bp_species_decontam_filtered.os %>%
              rename(os_filt = Observed_species), by = "Library_ID") %>%
  mutate(difference = Observed_species - os_filt) %>%
  left_join(., full_metadata %>%
...
ggsave("./06-publication/supplemental_figures/SXX15/filtered_species_counts_sub75bp.pdf", plot = filtered_sp, device = "pdf",
       scale = 1, width = 9, height = 5, units = c("in"), dpi = 300)
