...
full_metadata %>%
  ...
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = c("#f5767a", "#8dc6f7")) +
...
full_metadata %>%
  ...
  gather("Seq_depth", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "JAE","VLC","MID","CMB","RAD","KIL","ELR","IVE")) %>%
...
full_metadata %>%
  ...
  wilcox_test(data = ., Read_counts ~ Seq_depth) %>%
...
full_metadata %>%
  ...
  group_by(Time_period) %>%
  wilcox_test(data = ., Read_length ~ Seq_depth) %>%
  adjust_pvalue(method = "fdr") %>%
  add_significance("p.adj")
