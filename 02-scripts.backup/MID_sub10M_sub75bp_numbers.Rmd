---
title: "MID/CMB, Radcliffe, Kilteasheen, Iberian medieval calculus metadata and colors"
author: "Irina Velsko"
date: "`r format(Sys.time(), '%b %d, %Y')`"
output:
    github_document:
        fig_width: 8
        fig_height: 6
        toc: true
---

```{r load_libraries, echo = F, message = F}
library(knitr)
library(data.table)
library(tidyverse)
library(patchwork)
```

```{r setup}
knitr::opts_knit$set(root.dir = normalizePath(".."))
```


```{r}
# colors3 <- c("#f5767a", "#63f35e", "#8dc6f7") # "#5e63f3")
colors3 <- c("#311068", "#C83E73", "#41b6c4")
shapes3 <- c(21, 23, 24)

```

```{r}
# load the metadata file
load("./05-results.backup/Metadata_tables.RData")
```

Set the sample order for plotting
```{r}
sample_order <- full_metadata %>%
  select(Library_ID, Site_code) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  mutate(Site_code= fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  arrange(Site_code) %>%
  pull(Library_ID)
  
```

# Sequencing Depth
```{r seqdepth_groups}
full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(str_detect(Library_ID, "CMB|MID")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    ylab("No. reads")
  
full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(str_detect(Library_ID, "CS")) %>%
  filter(!str_detect(Library_ID, "L|D|N|SS")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    geom_hline(yintercept = 2500000, linetype = "dotted") +
    ylab("No. reads")

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(str_detect(Library_ID, "KT")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    geom_hline(yintercept = 2500000, linetype = "dotted") +
    ylab("No. reads")

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(str_detect(Library_ID, "ELR|IVE")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    ylab("No. reads")

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(str_detect(Library_ID, "JAE|VLC")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 10000000, linetype = "dotted") +
    geom_hline(yintercept = 20000000, linetype = "dotted") +
    ylab("No. reads")

```


```{r seqdepth_subs}
full_metadata %>%
  select(Library_ID, Site, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
  facet_wrap(~Seq_depth, nrow = 1) +
    ylab("No. reads")
  
full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  filter(Seq_depth == "No_reads_no_human") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[1]) +
    scale_shape_manual(values = shapes3[1]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
  facet_wrap(~Seq_depth, nrow = 1) +
    ylab("No. reads")
  

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  filter(Seq_depth == "No_reads_10M") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[2]) +
    scale_shape_manual(values = shapes3[2]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
  facet_wrap(~Seq_depth, nrow = 1) +
    ylab("No. reads")

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  filter(Seq_depth == "No_reads_75bp") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[3]) +
    scale_shape_manual(values = shapes3[3]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
  facet_wrap(~Seq_depth, nrow = 1) +
    ylab("No. reads")

```

```{r}
full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  filter(Seq_depth != "No_reads_75bp") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order),
         Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human", "No_reads_10M")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    ylab("No. reads")

full_metadata %>%
  select(Library_ID, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 2:ncol(.)) %>%
  filter(Seq_depth != "No_reads_10M") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order),
         Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human", "No_reads_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_counts, shape = Seq_depth, bg = Seq_depth)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = c("#311068", "#41b6c4")) +
    scale_shape_manual(values = c(21, 24)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 5000000, linetype = "dotted") +
    ylab("No. reads")


```


Summary box plots
```{r seqdepth_box}
tr_box_plot <- full_metadata %>%
  select(Library_ID, Site_code, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_depth", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE"),
         Read_counts = Read_counts / 1000000) %>%
  ggplot(., aes(x = Site_code, y = Read_counts, fill = Seq_depth)) +
    geom_boxplot(alpha = 0.5) +
    geom_jitter(alpha = 0.5, position = position_jitterdodge(jitter.width = 0.25), size = 1) +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5),
          text = element_text(size=16)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 10, linetype = "dotted") +
    labs(fill = "Dataset") +
    ylab("No. reads (x10^6)") +
    xlab("")

tr_box_plot

# one without modern calculus
full_metadata %>%
  select(Library_ID, Site_code, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN|JAE|VLC")) %>%
  gather("Seq_depth", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE"),
         Read_counts = Read_counts / 1000000) %>%
  ggplot(., aes(x = Site_code, y = Read_counts, fill = Seq_depth)) +
    geom_boxplot(alpha = 0.5) +
    geom_jitter(alpha = 0.5, position = position_jitterdodge(jitter.width = 0.25), size = 1) +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5),
          text = element_text(size=16)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 10, linetype = "dotted") +
    labs(fill = "Dataset") +
    ylab("No. reads (x10^6)") +
    xlab("")

```

```{r}

full_metadata %>%
  select(Library_ID, Site_code, Time_period, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_counts") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "No_reads_75bp") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., Read_counts ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_counts") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "No_reads_75bp") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,Read_counts ~ Seq_depth, p.adjust.method = "fdr")

```

```{r}
full_metadata %>%
  select(Library_ID, Site_code, Time_period, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_counts") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "No_reads_10M") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., Read_counts ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_counts") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "No_reads_10M") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,Read_counts ~ Seq_depth, p.adjust.method = "fdr")

```



# Average Read Length
```{r rl_groups}
full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(str_detect(Library_ID, "CMB|MID")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
    ggtitle("MID/CMB")
  
full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(str_detect(Library_ID, "CS")) %>%
  filter(!str_detect(Library_ID, "L|D|N|SS")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
    ggtitle("Radcliffe")

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(str_detect(Library_ID, "KT")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
    ggtitle("Kilteasheen")

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(str_detect(Library_ID, "ELR|IVE")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
    ggtitle("Iberia")

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(str_detect(Library_ID, "JAE|VLC")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
    ggtitle("Modern calculus")

```


```{r rl_subs}
full_metadata %>%
  select(Library_ID, Site, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 3:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)") +
  facet_wrap(~Seq_length, nrow = 1)
  
full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  filter(Seq_length == "seq_len_avg") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)")
  

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  filter(Seq_length == "seq_len_avg_10M") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[2]) +
    scale_shape_manual(values = shapes3[2]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg. read length (bp)")

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  filter(Seq_length == "seq_len_avg_75bp") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[3]) +
    scale_shape_manual(values = shapes3[3]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted")  +
    ylab("Avg. read length (bp)")

```

```{r}
full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  filter(Seq_length != "seq_len_avg_75bp") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 75, linetype = "dotted")  +
    ylab("Avg. read length (bp)")

full_metadata %>%
  select(Library_ID, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_length", 2:ncol(.)) %>%
  filter(Seq_length != "seq_len_avg_10M") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = Seq_length, bg = Seq_length)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = c("#311068", "#41b6c4")) +
    scale_shape_manual(values = c(21, 24)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 75, linetype = "dotted")  +
    ylab("Avg. read length (bp)")

```


Summary box plots
```{r rl_box}
rl_box_plot <- full_metadata %>%
  select(Library_ID, Site_code, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("Seq_length", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  ggplot(., aes(x = Site_code, y = Read_counts, fill = Seq_length)) +
    geom_boxplot(alpha = 0.5) +
    geom_jitter(alpha = 0.5, position = position_jitterdodge(jitter.width = 0.25), size = 1) +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5),
          text = element_text(size=16)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 75, linetype = "dotted") +
    labs(fill = "Dataset") +
    xlab("") +
    ylab("Avg. read length (bp)")

rl_box_plot

```

```{r}

full_metadata %>%
  select(Library_ID, Site_code, Time_period, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_length") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "seq_len_avg_75bp") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., Read_length ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_length") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "seq_len_avg_75bp") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,Read_length ~ Seq_depth, p.adjust.method = "fdr")

```

```{r}
full_metadata %>%
  select(Library_ID, Site_code, Time_period, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_length") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "seq_len_avg_10M") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., Read_length ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, seq_len_avg, seq_len_avg_10M, seq_len_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "Read_length") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "seq_len_avg_10M") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,Read_length ~ Seq_depth, p.adjust.method = "fdr")

```


# Average %GC 
```{r gc_groups}
full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(str_detect(Library_ID, "CMB|MID")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
    ggtitle("MID/CMB")
  
full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(str_detect(Library_ID, "CS")) %>%
  filter(!str_detect(Library_ID, "L|D|N|SS")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
    ggtitle("Radcliffe")

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(str_detect(Library_ID, "KT")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
    ggtitle("Kilteasheen")

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(str_detect(Library_ID, "ELR|IVE")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
    ggtitle("Iberia")

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(str_detect(Library_ID, "JAE|VLC")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp")) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
    ggtitle("Modern calculus")

```


```{r gc_subs}
full_metadata %>%
  select(Library_ID, Site, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 3:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC") +
  facet_wrap(~GC, nrow = 1)
  
full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  filter(GC == "gc_avg") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC")
  

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  filter(GC == "gc_avg_10M") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[2]) +
    scale_shape_manual(values = shapes3[2]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    ylab("Avg %GC")

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  filter(GC == "gc_avg_75bp") %>%  
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 3, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3[3]) +
    scale_shape_manual(values = shapes3[3]) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted")  +
    ylab("Avg %GC")

```

```{r}
full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  filter(GC != "gc_avg_75bp") %>%
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    scale_shape_manual(values = shapes3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted")  +
    ylab("Avg %GC")

full_metadata %>%
  select(Library_ID, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_length", 2:ncol(.)) %>%
  filter(GC != "gc_avg_10M") %>%
  mutate(Library_ID= fct_relevel(Library_ID, sample_order)) %>%
  ggplot(., aes(x = Library_ID, y = Read_length, shape = GC, bg = GC)) +
    geom_point(size = 2, color = "grey50") +
    theme_minimal() +
    scale_fill_manual(values = c("#311068", "#41b6c4")) +
    scale_shape_manual(values = c(21, 24)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          text = element_text(size=12)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=8)) +
    geom_hline(yintercept = 50, linetype = "dotted")  +
    ylab("Avg %GC")

```


Summary box plots
```{r gc_box}
gc_box_plot <- full_metadata %>%
  select(Library_ID, Site_code, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  gather("GC", "Read_counts", 3:ncol(.)) %>%
  mutate(GC = fct_relevel(GC, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  ggplot(., aes(x = Site_code, y = Read_counts, fill = GC)) +
    geom_boxplot(alpha = 0.5) +
    geom_jitter(alpha = 0.5, position = position_jitterdodge(jitter.width = 0.25), size = 1) +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5),
          text = element_text(size=16)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=12)) +
    geom_hline(yintercept = 50, linetype = "dotted") +
    labs(fill = "Dataset") +
    xlab("") +
    ylab("Avg. GC (%)")

gc_box_plot

```

```{r}

full_metadata %>%
  select(Library_ID, Site_code, Time_period, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "GC") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "gc_avg_75bp") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., GC ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "GC") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "gc_avg_75bp") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,GC ~ Seq_depth, p.adjust.method = "fdr")

```

```{r}

full_metadata %>%
  select(Library_ID, Site_code, Time_period, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "GC") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "gc_avg_10M") %>%
  group_by(Time_period) %>%
  pairwise_wilcox_test(data = ., GC ~ Seq_depth, p.adjust.method = "fdr")

full_metadata %>%
  select(Library_ID, Site_code, Time_period, gc_avg, gc_avg_10M, gc_avg_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN")) %>%
  pivot_longer(!Library_ID:Time_period, names_to = "Seq_depth", values_to = "GC") %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "gc_avg","gc_avg_10M","gc_avg_75bp"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
  mutate(Time_period = as.character(Time_period)) %>%
  filter(Seq_depth != "gc_avg_10M") %>%
  group_by(Site_code) %>%
  pairwise_wilcox_test(data = .,GC ~ Seq_depth, p.adjust.method = "fdr")

```

```{r}
tr_rl_gc_figure <- tr_box_plot + rl_box_plot + gc_box_plot +
  plot_layout(nrow = 3) + 
  plot_annotation(tag_levels = 'A')

tr_rl_gc_figure

ggsave("./06-publication/supplemental_figures/SXX10/panel_parts/tr_rl_gc_figure.pdf", plot = tr_rl_gc_figure, device = "pdf",
       scale = 1, width = 13, height = 15, units = c("in"), dpi = 300)

```



























