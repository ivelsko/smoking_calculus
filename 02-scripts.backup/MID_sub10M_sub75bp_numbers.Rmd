...
sample_order <- full_metadata %>%
  ...
  mutate(Site= fct_relevel(Site, "Jaen","VLC","Radcliffe","CMB","Middenbeemster","ELR","IVE","Kilteasheen")) %>%
  arrange(Site) %>%
  pull(Library_ID)
...
full_metadata %>%
  ...
  ggplot(., aes(x = Site, y = Read_counts, fill = Seq_depth)) +
    geom_violin() +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
full_metadata %>%
  select(Library_ID, Site, No_reads_no_human, No_reads_10M, No_reads_75bp) %>%
  filter(!str_detect(Library_ID, "EXB|LIB|CSS|CSL|CSD|CSN|JAE")) %>%
  gather("Seq_depth", "Read_counts", 3:ncol(.)) %>%
  mutate(Seq_depth = fct_relevel(Seq_depth, "No_reads_no_human","No_reads_10M","No_reads_75bp"),
         Site = fct_relevel(Site, "Jaen","Middenbeemster","CMB","Radcliffe","Kilteasheen","ELR","IVE")) %>%
  ggplot(., aes(x = Site, y = Read_counts, fill = Seq_depth)) +
...
full_metadata %>%
  ...
  mutate(Seq_length = fct_relevel(Seq_length, "seq_len_avg","seq_len_avg_10M","seq_len_avg_75bp"),
         Site = fct_relevel(Site, "Jaen","Middenbeemster","CMB","Radcliffe","Kilteasheen","ELR","IVE")) %>%
  ggplot(., aes(x = Site, y = Read_counts, fill = Seq_length)) +
    geom_boxplot() +
    geom_jitter(alpha = 0.3, position = position_jitterdodge(jitter.width = 0.1), size = 1) +
    theme_minimal() +
    scale_fill_manual(values = colors3) +
