...
taxatable <- load_taxa_table(nt_raw) # %>% print()
...
rank_burnin_result <- hard_burnin_filter(curves, percent_threshold = 55, rank_burnin = 0.4) # percent_threshold = 50, rank_burnin = 0.7 allows only the highest ExtractionBlank line to turn turquoise, and the 2 library samples that really look like samples stay turquoise
...
fwrite(discard_list, "./05-results.backup/cuperdec_nt_poor_samples.tsv", sep = "\t", quote = F)
