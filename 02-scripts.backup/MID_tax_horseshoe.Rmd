...
# Make sure there's no batch effect in the horseshoe by coloring by either extraction batch or library batch

malt_pca_values %>%
ggplot(., aes(PC1, PC2, color = Library_batch, shape = Region)) +
  geom_text(aes(label = Library_ID), size = 3) +
  theme_minimal()

horseshoe <- ggplot(malt_pca_values, aes(PC1, PC2, color = Library_batch, shape = Region)) + # Extraction_batch
  geom_point(size = 2) +
  theme_minimal()
