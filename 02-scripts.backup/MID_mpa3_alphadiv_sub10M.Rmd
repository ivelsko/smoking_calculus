...
```{r}
mpa3_sub10M_contaminants <- fread("./05-results.backup/contaminant_species_radcliffe_mpa3_sub10M.tsv", sep = "\t") %>%
...
mpa3_sub10M_species_decontam <- mpa3_sub10M_sp %>%
  anti_join(., mpa3_sub10M_contaminants)
...
poor_samples_mpa3 <- fread("./05-results.backup/cuperdec_mpa3_sub10M_poor_samples.tsv") %>%
  pull(Sample)
...
presabs_table <- mpa3_sub10M_species_decontam %>%
...
mpa3_sub10M_species_decontam.os <- presabs_table %>%
  adorn_totals(., where = "col", name = "Observed_species") %>%
  select(Observed_species) %>%
  rownames_to_column("Library_ID") %>%
  filter(!str_detect(Library_ID, "EXB|LIB")) 

plot_box_whisker(mpa3_sub10M_species_decontam.os, "Observed_species", "Site_code", "Site_code")
plot_box_whisker(mpa3_sub10M_species_decontam.os, "Observed_species", "Region", "Region")

# os_plot <-mpa3_sub10M_species_decontam.os %>%
#   inner_join(full_metadata %>%
...
# calculate observed species for each sample with janitor 'adorn_totals'
mpa3_sub10M_species_decontam_filtered.os <- presabs_table_f %>%
  adorn_totals(., where = "col", name = "Observed_species") %>%
  select(Observed_species) %>%
  rownames_to_column("Library_ID")  

plot_box_whisker(mpa3_sub10M_species_decontam_filtered.os, "Observed_species", "Site_code", "Site_code")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.os, "Observed_species", "Time_period", "Region")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.os, "Observed_species", "Region", "Time_period")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.os, "Observed_species", "Time_period", "Time_period")


filtered_os <- mpa3_sub10M_species_decontam_filtered.os %>%
  inner_join(., full_metadata, by = "Library_ID") %>%
  ggplot(., aes(x = Site_code, y = Observed_species, fill = Site_code)) +
  geom_boxplot(color = "grey30") +
...
plot1 <-  mpa3_sub10M_species_decontam_filtered.os %>%
  inner_join(., full_metadata, by = "Library_ID") %>%
...
# compare difference in the number of species between filtered and unfiltered tables
mpa3_sub10M_species_decontam.os %>%
  full_join(., mpa3_sub10M_species_decontam_filtered.os %>%
              rename(os_filt = Observed_species), by = "Library_ID") %>%
  mutate(difference = Observed_species - os_filt)
...
mpa3_sub10M_species_decontam.os %>%
  ...
  left_join(., full_metadata %>%
              select(Library_ID, Time_period, Site_code), by = "Library_ID") %>%
  group_by(Time_period, Site_code, difference) %>%
  mutate(Time_period = fct_relevel(Time_period, "Medieval","Industrial","Modern")) %>%
  count(name = "No_samples") %>%
  ggplot(., aes(x = difference, y = No_samples, fill = Site_code)) +
    geom_bar(stat = "identity", position = position_dodge2(width = 0.9, preserve = "single"), color = "grey30", size = 0.2) +
    geom_text(aes(label = No_samples), position=position_dodge(width=0.9), vjust=-0.25, size = 3) +
    scale_fill_manual(values = Site_code_colors) +
    theme_minimal(base_size = 12) +
    scale_x_continuous(breaks=scales::pretty_breaks(n=8)) +
    scale_y_continuous(breaks=scales::pretty_breaks(n=10)) +
    theme(axis.text.x = element_text(vjust = 0.5)) +
    xlab("No. species removed by filtering") +
    ylab("No. Samples")

# compare difference in the number of species between filtered and unfiltered tables
filtered_sp <- mpa3_sub10M_species_decontam.os %>%
  full_join(., mpa3_sub10M_species_decontam_filtered.os %>%
              rename(os_filt = Observed_species), by = "Library_ID") %>%
  mutate(difference = Observed_species - os_filt) %>%
  left_join(., full_metadata %>%
  ...
  group_by(Time_period, Site_code, difference) %>%
  mutate(Time_period = fct_relevel(Time_period, "Medieval","Industrial","Modern"),
         Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","JAE","VLC")) %>%
  count(name = "No_samples") %>%
  arrange(Time_period) %>%
  ggplot(., aes(x = difference, y = No_samples, fill = Site_code)) +
    geom_bar(stat = "identity", position = position_dodge2(width = 0.9, preserve = "single"), color = "grey30", size = 0.2) +
...
ggsave("./06-publication/supplemental_figures/SXX15/filtered_species_counts_sub10M.pdf", plot = filtered_sp, device = "pdf",
       scale = 1, width = 9, height = 5, units = c("in"), dpi = 300)
...
mpa3_sub10M_species_decontam.matrix <- as.matrix(mpa3_sub10M_species_decontam %>%
  select(-one_of(outliers_mpa3)) %>%
  select(-one_of(poor_samples_mpa3)) %>%
  select(-matches("EXB|LIB|CSN|CSL|CSD|CSS")) %>%
  gather("Library_ID","Counts",2:ncol(.)) %>%
...
mpa3_sub10M_species_decontam.shannon <- as.data.frame(diversity(mpa3_sub10M_species_decontam.matrix, index = "shannon"))
names(mpa3_sub10M_species_decontam.shannon)[1] <- "Shannon_index"

# merge with the metadata file and plot
mpa3_sub10M_species_decontam.shannon <- mpa3_sub10M_species_decontam.shannon %>%
  rownames_to_column("Library_ID")

plot_box_whisker(mpa3_sub10M_species_decontam.shannon, "Shannon_index", "Site_code", "Site_code")
plot_box_whisker(mpa3_sub10M_species_decontam.shannon, "Shannon_index", "Region", "Region")

shannon_sp_plot <-mpa3_sub10M_species_decontam.shannon %>%
  inner_join(full_metadata %>%
               select(Library_ID, Site_code, Region, Time_period, Sample_or_Control, 
                      Periodontal_disease_present, Caries_present, Sex, Age), by = "Library_ID") %>%
...
mpa3_sub10M_species_decontam_filtered.matrix <- as.matrix(mpa3_sub10M_species_decontam %>%
  select(-one_of(outliers_mpa3)) %>%
  select(-one_of(poor_samples_mpa3)) %>%
  select(-matches("EXB|LIB|CSN|CSL|CSD|CSS")) %>%
  adorn_totals(where = "col", name = "Sum_species.decontam") %>%
...
mpa3_sub10M_species_decontam_filtered.shannon <- as.data.frame(diversity(mpa3_sub10M_species_decontam_filtered.matrix, index = "shannon"))
names(mpa3_sub10M_species_decontam_filtered.shannon)[1] <- "Shannon_index"

# merge with the metadata file and plot
mpa3_sub10M_species_decontam_filtered.shannon <- mpa3_sub10M_species_decontam_filtered.shannon %>%
  rownames_to_column("Library_ID")

plot_box_whisker(mpa3_sub10M_species_decontam_filtered.shannon, "Shannon_index", "Site_code", "Site_code")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.shannon, "Shannon_index", "Time_period", "Region")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.shannon, "Shannon_index", "Region", "Time_period")
plot_box_whisker(mpa3_sub10M_species_decontam_filtered.shannon, "Shannon_index", "Time_period", "Time_period")

plot1 <-  mpa3_sub10M_species_decontam_filtered.shannon %>%
  inner_join(., full_metadata, by = "Library_ID") %>%
...
```{r evenness_pielou}
evenness.species <- diversity(mpa3_sub10M_species_decontam.matrix)
evenness.species <- as.data.frame(evenness.species/log(specnumber(mpa3_sub10M_species_decontam.matrix)))
...
evenness.species_filtered <- diversity(mpa3_sub10M_species_decontam_filtered.matrix)
evenness.species_filtered <- as.data.frame(evenness.species_filtered/log(specnumber(mpa3_sub10M_species_decontam_filtered.matrix)))
...
# calculate observed species for each sample with janitor 'adorn_totals'
mpa3_sub10M_species_decontam_filtered.alpha <- presabs_table_f %>%
  adorn_totals(., where = "col", name = "Observed_species") %>%
  select(Observed_species) %>%
  rownames_to_column("Library_ID") 
...
mpa3_sub10M_species_decontam_filtered.alpha <- mpa3_sub10M_species_decontam_filtered.alpha %>%
  full_join(., mpa3_sub10M_species_decontam_filtered.shannon %>%
...
os_site <- plot_box_whisker(mpa3_sub10M_species_decontam_filtered.alpha, "Observed_species", "Site_code", "Site_code")
os_site
shannon_site <- plot_box_whisker(mpa3_sub10M_species_decontam_filtered.alpha, "Shannon_index", "Site_code", "Site_code")
shannon_site
pe_site <- plot_box_whisker(mpa3_sub10M_species_decontam_filtered.alpha, "Pileou_evenness", "Site_code", "Site_code")
pe_site
...
mpa3_sub10M_species_decontam_filtered.alpha_meta <- mpa3_sub10M_species_decontam_filtered.alpha %>%
...
fwrite(mpa3_sub10M_species_decontam_filtered.alpha, "~/archgen/microbiome_calculus/smoking_calculus/05-results.backup/mpa3_alpha_div_numbers_sub10M.tsv", sep = "\t", quote = F)
...
Envmt <- colnames(mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
...
pvalues_envmt <- mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
  ...
  bind_rows(., mpa3_sub10M_species_decontam_filtered.alpha_meta %>% 
...
# fwrite(species_kruskal_pvals, "./06-publication/supplemental_tables/alphadiv_kruskal_wallace_mpa3_sub10M_sp.tsv", sep = "\t", quote = F)
...
# fwrite(IC_species_p_es, "./06-publication/supplemental_tables/alphadiv_wilcox_mpa3_sub10M_sp.tsv", sep = "\t", quote = F)
# fwrite(IC_species_p_es, "~/Desktop/mid_alpha_stats.tsv", sep = "\t", quote = F)

```

```{r raincloud_plots_os_tp}
sub10M_tp_raincloud <- mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
  mutate(Time_period = fct_relevel(Time_period, "Medieval","Industrial","Modern")) %>%
ggplot(., aes(x = Time_period, y = Observed_species, fill = Time_period)) + 
...
sub10M_sc_raincloud <- mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
...
sub10M_tp_sh_raincloud <- mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
  mutate(Time_period = fct_relevel(Time_period, "Medieval","Industrial","Modern")) %>%
  ggplot(., aes(x = Time_period, y = Shannon_index, fill = Time_period)) + 
  ## add half-violin from {ggdist} package
  ggdist::stat_halfeye(
...
sub10M_tp_sh_raincloud

```

```{r raincloud_plots_sh_site}
sub10M_sc_sh_raincloud <- mpa3_sub10M_species_decontam_filtered.alpha_meta %>%
  mutate(Site_code = fct_relevel(Site_code, "IVE","ELR","KIL","RAD","CMB","MID","VLC","JAE")) %>%
ggplot(., aes(x = Site_code, y = Shannon_index, fill = Site_code)) + 
  ## add half-violin from {ggdist} package
  ggdist::stat_halfeye(
...
sub10M_sc_sh_raincloud

```

```{r, eval = F}
sub10M_alpha_figure <- sub10M_tp_raincloud + sub10M_tp_sh_raincloud +sub10M_sc_raincloud + sub10M_sc_sh_raincloud +
  plot_layout(nrow = 2) + 
  plot_annotation(tag_levels = 'A')

sub10M_alpha_figure

ggsave("./06-publication/supplemental_figures/S7/sub10M_alpha_figure.pdf", plot = sub10M_alpha_figure, device = "pdf",
       scale = 1, width = 10, height = 7, units = c("in"), dpi = 300)

ggsave("./06-publication/supplemental_figures/S7/sub10M_alpha_figure.png", plot = sub10M_alpha_figure, device = "png",
       scale = 1, width = 10, height = 7, units = c("in"), dpi = 300)
