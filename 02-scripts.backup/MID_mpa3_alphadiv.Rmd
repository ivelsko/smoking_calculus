...
plot_box_whisker(mpa3_species_decontam.os, "Observed_species", "Site", "Site")
...
plot_box_whisker(mpa3_species_decontam_filtered.os, "Observed_species", "Site", "Site")
...
plot_box_whisker(mpa3_species_decontam.shannon, "Shannon_index", "Site", "Site")
...
plot_box_whisker(mpa3_species_decontam_filtered.shannon, "Shannon_index", "Site", "Site")
...
os_site <- plot_box_whisker(mpa3_species_decontam_filtered.alpha, "Observed_species", "Site", "Site")
os_site
shannon_site <- plot_box_whisker(mpa3_species_decontam_filtered.alpha, "Shannon_index", "Site", "Site")
shannon_site
pe_site <- plot_box_whisker(mpa3_species_decontam_filtered.alpha, "Pileou_evenness", "Site", "Site")
pe_site
...
Envmt <- colnames(mpa3_species_decontam_filtered.alpha_meta %>%
                    select(Site_code, Region, Time_period, 
...
mpa3_species_decontam_top50_mpa3_clr_smp_order <- mpa3_species_decontam_top50_mpa3_clr %>%
  ...
  gather("Species","CLR",2:ncol(.)) %>%
  spread(Species,CLR) %>%
  mutate(SmpOrder = as.character(seq(1,181))) %>%  
...
mpa3_species_decontam_top50_mid_clr_smp_order <- mpa3_species_decontam_top50_mid_clr %>%
  ...
  gather("Species","CLR",2:ncol(.)) %>%
  spread(Species,CLR) %>%
  mutate(SmpOrder = as.character(seq(1,63))) %>%  
  select(Library_ID, SmpOrder, everything()) %>%
...
mpa3_species_decontam_top50_each_clr_long <- mpa3_species_decontam_top50_each_clr %>%
  ...
  inner_join(., Site_colors %>%
               ...
               rename(Color = value) %>%
               mutate(Site = c("Middenbeemster", "CMB", "Radcliffe", "Kilteasheen",
                                   "ELR", "IVE","Jaen","VLC","Extraction_blank","Library_blank")), by = "Site") %>%
