...
plot_pca_bi <- function(df, pc1, pc2, metadata_group, columntitle) {
    ...
    pca_plot_bi <- ggplot(df_Y, aes(x = get(paste(pc1)), y = get(paste(pc2)))) +
      geom_point(size = 3.5, aes(shape = get(paste(metadata_group)), fill = get(paste(metadata_group)))) +
      ...
      geom_label_repel(data = pws_10,
                   ...
                   size = 4.0, colour = "grey20", label.padding = 0.2, force = 5, max.overlaps = 14) +
      ...
      geom_label_repel(data = neg_10,
                   aes(x = get(paste(pc1)), y = get(paste(pc2)), label = Taxon),
                   size = 4.0, colour = "grey20", label.padding = 0.2, max.overlaps = 14) +
...
ggsave("./06-publication/main_figures/Figure_5/biplot.pdf", plot = mpa3_species_decontam_filtered_nocontrols_biplot_121,
       device = "pdf", scale = 1, width = 10, height = 8, units = c("in"), dpi = 300)
...
# ggsave("./06-publication/main_figures/Figure_5/beta_div_all.pdf", plot = beta_div,
...
all_pca_values_select <- all_pca_values %>%
  select(Library_ID,PC1, PC2, seq_len_avg, gc_avg, No_reads_total, Time_period_n, Site_code_n) %>%
...
form <- ~ PC1 + PC2 + `Seq length` + GC + `Total reads` + `Time period` + Site
...
rstatix::cor_test(., PC1, PC2, PC3, seq_len_avg, gc_avg, No_reads_total, Time_period_n, Site_code_n, method="pearson")
...
```{r, eval = F}
svg(file = "./06-publication/main_figures/Figure_5/all_corr_plot.svg") 
