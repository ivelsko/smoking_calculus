...
# paths_beta_div
# 
# ggsave("./06-publication/main_figures/Figure_3/h3_paths_beta_div_high.pdf", plot = paths_beta_div,
#        device = "pdf", scale = 1, width = 15, height = 15, units = c("in"), dpi = 300)


h3_pcas <- pipenotch + pd + gc + sg +
  plot_layout(ncol = 2)

h3_pcas

ggsave("./06-publication/main_figures/Figure_3/h3_paths_beta_div.pdf", plot = h3_pcas,
       device = "pdf", scale = 1, width = 15, height = 11.5, units = c("in"), dpi = 300)
...
ggsave("./06-publication/main_figures/Figure_3/h3_paths_biplots.pdf", plot = h3biplots,
       device = "pdf", scale = 1, width = 10, height = 9.25, units = c("in"), dpi = 300)
...
h3_paths_pca_values_select <- h3_paths_pca_values %>%
  select(Library_ID, PC1, PC2, PC1_sp, PC2_sp, Percent_pws_pc1, Percent_neg_pc1, Percent_pws_pc2, Percent_neg_pc2, seq_len_avg, gc_avg, No_reads_total, Age_n, Sex_n, Pipenotch_n, Min_no_Pipe_notches, `%AMTL`, `%teeth_with_caries`, calc_sup_SI_score, Peridontal_affect_score, `%_positions_with_Periodontal`, Max_Perio_Score, Calculus_extracted_mg, `extract_conc_ng/mg`, `extract_conc_ng/uL`, `extract_conc_after_storage_ng/uL`, `DNA_molecules_per_library_x10^6`,Calc_sub_SI_score, `%teeth_with_calc`, `%_positions_perioapical`) %>%
  ...
  select(Library_ID, PC1, PC2, Age, Sex, Pipenotch, `Min no. pipenotch`, `% AMTL`, `% Caries`, `Supra calc score`, `Sub calc score`, `% Calculus`, `Perio score`, `% Perio`, `Max perio score`, `% Perioapical`, `Extracted weight (mg)`, `Pre-storage [extract] (ng/mg)`, `Pre-storage [extract] (ng/uL)`, `Post-storage [extract] (ng/uL)`, `[Library] (molecules)`, `Seq length`, GC, `Total reads`, `Species PC1`, `Species PC2`, `Species % PC1+`, `Species % PC1-`, `Species % PC2+`, `Species % PC2-`)

# select variables
form <- ~ PC1 + PC2 + Age + Sex + Pipenotch + `Min no. pipenotch` + `% AMTL` + `% Caries` + `Supra calc score` + `Sub calc score` + `% Calculus` + `Perio score` + `% Perio` + `Max perio score` + `% Perioapical` + `Extracted weight (mg)` + `Pre-storage [extract] (ng/mg)` + `Pre-storage [extract] (ng/uL)` + `Post-storage [extract] (ng/uL)` + `[Library] (molecules)` + `Seq length` + GC + `Total reads` + `Species PC1` + `Species PC2` +`Species % PC1+` + `Species % PC1-` + `Species % PC2+` + `Species % PC2-`
...
rstatix::cor_test(., PC1, PC2, Age, Sex, Pipenotch, `Min no. pipenotch`, `% AMTL`, `% Caries`, `Supra calc score`, `Sub calc score`, `% Calculus`, `Perio score`, `% Perio`, `Max perio score`, `% Perioapical`, `Extracted weight (mg)`, `Pre-storage [extract] (ng/mg)`, `Pre-storage [extract] (ng/uL)`, `Post-storage [extract] (ng/uL)`, `[Library] (molecules)`, `Seq length`, GC, `Total reads`, `Species PC1`, `Species PC2`, `Species % PC1+`, `Species % PC1-`, `Species % PC2+`, `Species % PC2-`, method="pearson")
...
```{r}
svg(file = "./06-publication/main_figures/Figure_3/mid_h3_path_corr_plot.svg") 
# pdf(file = "./06-publication/main_figures/Figure_3/mid_h3_path_corr_plot.pdf") 
